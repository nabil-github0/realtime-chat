...
const Layout = async ({children}: LayoutProps) => {
 ...
 return (
    <div className='w-full flex h-screen'>
        ...
        {friends.length > 0 ? (<div className='text-xs font-semibold leading-6 text-gray-400'>You Chats</div>) : null}
                    ...
                    <ul role='list' className='-mx-2 mt-2 space-y-1'>
                        {sidebarOptions.map((option) => {
                            ...
                            return (
                                <li key={option.id}>
                                    <Link 
                                    className='text-gray-700 hover:text-indigo-600 hover:bg-gray-50 group flex gap-3 rounded-e-md p-2 text-sm leading-6 font-semibold'
                                    href={option.href}
                                    > 
